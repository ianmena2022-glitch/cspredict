...
import { RefreshCw, TrendingUp, Trophy, Info, DollarSign, History } from 'lucide-react';
    ...
    <div className="space-y-4 max-w-2xl">
      <div className="bg-[#1a2235] border border-[#1e2d45] rounded-xl p-5">
        ...
        <div className="space-y-2 text-sm text-slate-400">
          <div className="flex gap-3"><span className="text-green-400 font-mono w-12">5 min</span> Actualiza partidos y cuotas de 1xbet desde PandaScore + TheOddsAPI</div>
          <div className="flex gap-3"><span className="text-blue-400 font-mono w-12">1 hora</span> Verifica resultados y actualiza el bankroll automáticamente</div>
          <div className="flex gap-3"><span className="text-purple-400 font-mono w-12">Siempre</span> Guarda cada predicción en la base de datos para calcular el ROI real</div>
        </div>
      </div>
      ...
      <div className="bg-[#1a2235] border border-[#1e2d45] rounded-xl p-5">
        <h2 className="font-bold mb-3">Kelly Criterion</h2>
        ...
        <ul className="text-sm text-slate-400 space-y-1">
          ...
          <li><span className="text-white">fracción</span> = 0.25 (Quarter Kelly, más conservador)</li>
          <li className="pt-1 text-xs text-slate-500">Máximo 5% del bankroll por apuesta independientemente del Kelly calculado</li>
        </ul>
      </div>

      <div className="bg-[#1a2235] border border-[#1e2d45] rounded-xl p-5">
        <h2 className="font-bold mb-2">Pinnacle como referencia</h2>
        <p className="text-sm text-slate-400">
          Cuando hay cuotas de Pinnacle disponibles, el edge se calcula contra Pinnacle
          (el bookmaker más sharp del mundo) en vez de contra 1xbet. Esto da una medida
          más honesta del valor real de la apuesta.
        </p>
        <div className="mt-3 bg-[#111827] rounded p-3 font-mono text-xs text-slate-300">
          edge = prob_nuestra - prob_Pinnacle
        </div>
      </div>
...
export default function App() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [m, r, s, br, st, hist, cfg] = await Promise.all([
        getMatches(), getRankings(), getStatus(),
      ...
      setStatus(s);
      setBankroll(br);
      setStats({ ...st, roi: br?.stats?.roi ?? st?.roi });
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a]">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        {tab === 'bankroll' && (
          <BankrollDashboard
            stats={{ ...stats, roi: bankrollData ? ((bankrollData.amount - (bankrollData.history?.[0]?.amount || 100)) / (bankrollData.history?.[0]?.amount || 100) * 100).toFixed(2) : 0 }}
        ...
        {tab === 'rankings' && (
          <div className="max-w-2xl">
            {loading ? <div className="bg-[#1a2235] border border-[#1e2d45] rounded-xl h-96 animate-pulse" /> : <Rankings rankings={rankings} />}
          </div>
        )}
